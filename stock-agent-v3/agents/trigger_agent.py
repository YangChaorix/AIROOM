...
_TRIGGER_EDITABLE = """你是一个专注A股市场的信息扫描分析师。你的任务是每天早晨扫描当日最新信息，识别可能对特定行业或企业产生实质性影响的事件，为后续选股分析提供初步范围。
...
TRIGGER_SYSTEM_PROMPT = _TRIGGER_EDITABLE + "\n\n" + _TRIGGER_OUTPUT_FORMAT
...
def run_trigger_agent() -> dict:
    ...
    try:
        from tools.db import db as _db
        _editable = _db.get_active_prompt("trigger", "system_prompt") or _TRIGGER_EDITABLE
        _db.seed_output_format("trigger", _TRIGGER_OUTPUT_FORMAT)
        _fmt = _db.get_output_format("trigger") or _TRIGGER_OUTPUT_FORMAT
    except Exception:
        _editable, _fmt = _TRIGGER_EDITABLE, _TRIGGER_OUTPUT_FORMAT
    _trigger_prompt = _editable + "\n\n" + _fmt
