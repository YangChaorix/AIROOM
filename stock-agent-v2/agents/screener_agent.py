...
def _collect_company_data(company_name: str, stock_code: str) -> dict:
    ...
    try:
        ...
        data["基本信息"] = info
        logger.debug(f"    基本信息: 行业={info.get('行业','N/A')} 总市值={info.get('总市值(亿)','N/A')}亿 "
    ...
    try:
        ...
        logger.debug(f"    最新财务: 净利率={recent.get('净利率','N/A')} "
    ...
    try:
        ...
        logger.debug(f"    私募+个人={cs.get('私募+个人持股(%)','N/A')}% "
    ...
    try:
        ...
        data["技术突破D5"] = tech
        logger.debug(f"    量比={tech.get('量比(3日/20日)','N/A')}x "
...
- 每个维度理由控制在15字以内，推荐理由控制在60字以内。"""
        ...
        for item in items:
            ...
            logger.info(f"  [{batch_label}] #{item.get('rank','?')} {item.get('name','')}({item.get('code','')}) "
    ...
    batch2 = _call_batch("第11名到第20名（Top 11~20），即综合得分排第11到第20的企业", "第2批")
