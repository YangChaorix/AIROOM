...
REVIEW_SYSTEM_PROMPT = """你是一个A股市场的每日复盘分析师。在每天收盘后运行，独立分析市场真实表现，并与当日推送的选股结果交叉验证。
...
def run_review_agent(daily_push: dict | None = None) -> dict:
    """
    运行复盘Agent：
    1. 获取市场行情数据（涨跌幅榜、板块数据）
    2. 读取当日推送记录
    3. 调用LLM生成复盘日报
    4. 返回markdown格式日报
    ...
    try:
        market_data = get_market_movers(top_n=50)
        ov = market_data.get("市场概况", {})
        gainers = market_data.get("涨幅前50", [])
        losers = market_data.get("跌幅前50", [])
        logger.info(f"市场概况: 上涨={ov.get('上涨家数','N/A')} 下跌={ov.get('下跌家数','N/A')} "
    ...
    if daily_push:
        ...
        if push_top20:
            logger.info(f"  推送股票: {[(s.get('name'), s.get('code')) for s in push_top20[:5]]}...")
    ...
    sector_text = json.dumps(sector_data[:20] if sector_data else [], ensure_ascii=False, indent=2)

    friday_note = "\n注意：今天是周五，请额外输出【本周经验积累】部分。" if is_friday else ""

    human_content = f"""今日日期：{today}（{'周五' if is_friday else '工作日'}）{friday_note}
