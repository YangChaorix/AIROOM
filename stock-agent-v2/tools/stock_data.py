    ...
    try:
        ...
        try:
            ...
            hist_df = ak.stock_zh_a_hist(
                ...
                start_date=_fmt_date(start_date), end_date=_fmt_date(end_date),
                adjust="qfq",
            )
    ...
    try:
        ...
        if profit_df is not None and not profit_df.empty:
            ...
            for _, row in recent.iterrows():
                record = {col: (str(row[col]) if not (isinstance(row[col], float) and pd.isna(row[col])) else "N/A")
...
def get_historical_volume(stock_code: str, days: int = 30) -> dict:
    ...
    try:
        ...
        col_map = {"成交量": "volume", "换手率": "turnover_rate", "涨跌幅": "pct_change",
                   "收盘": "close", "最高": "high"}
        ...
        ma20_vol = float(df["volume"].tail(20).mean()) if len(df) >= 20 else float(df["volume"].mean())
        ...
        max_60d = float(df["close"].tail(60).max()) if "close" in df.columns and len(df) >= 10 else None
